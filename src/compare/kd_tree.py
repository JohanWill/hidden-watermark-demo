...
class KDTree(object):
    ...
    def knn_algo(self, item, k=1):
        ...
        if self.length <= k:
            ...
            for element in self.transfer_list(self.root):
                distance = np.sqrt(sum((item - np.array(element['item'])) ** 2))  # 测试点与最近点之间的距离
                node_list.append([distance, element['item'], element['label']])
        ...
        if node == None:  # 空树
            return None
        # logger.info('比较点:%s\n靠近点:%s' % (item, node.item))
        distance = np.sqrt(sum((item - node.item) ** 2))  # 测试点与最近点之间的距离
        least_dis = distance
        # 返回上一个父结点，判断以测试点为圆心，distance为半径的圆是否与父结点分隔超平面相交，
        # 若相交，则说明父结点的另一个子树可能存在更近的点
        ...
        if node.left_child != None:
            ...
            if k > len(node_list) or left_dis < least_dis:
                ...
                node_list.sort(key=lambda item: item[0])  # 对结点列表按距离排序
        ...
        while True:
            ...
            if k > len(node_list) or abs(item[parent.dim] - parent.item[parent.dim]) < least_dis:
                ...
                if other_child != None:
                    if item[parent.dim] - parent.item[parent.dim] <= 0:     # 测试点在该子结点超平面的左侧
    ...
    def left_search(self, item, node, nodeList, k):
        ...
        :return: 结点列表
        '''
        nodeList.sort(key=lambda item: item[0])  # 对结点列表按距离排序
...
if __name__ == '__main__':
    ...
    # logger.info('data',np.hstack((dataArray,label)))
    kd_tree = KDTree(dataArray,label)
    # kd_dict = kd_tree.transfer_dict(kd_tree.root)
    # logger.info('kd_dict:',kd_dict)
    # kd_list = kd_tree.transfer_list(kd_tree.root)
    # logger.info('kd_list',kd_list)
    # for i in kd_list:
    #     logger.info(i)
    # node = kd_tree.find_nearest_neighbour([12,7])
    # logger.info('%s最近的叶结点:%s'%([12,7],node.item))
    t2 = time.time()
    node_list = kd_tree.knn_algo([12,7],k=5)
    logger.info('点%s的最接近的前k个点为:%s'%([12,7], node_list))
    # logger.info('点%s的标签:%s'%([12,7],label))
    t3 = time.time()
    logger.info('创建树耗时：',t2-t1)
    logger.info('搜索前k个最近邻点耗时：',t3-t2)
